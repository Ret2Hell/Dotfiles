...
local function translate_obsidian_datetime_format(format)
	local tokens = {
		...
		{ "ss", "%S" },
		{ "A", "%p" },
		{ "a", "__RET2HELL_OBSIDIAN_LOWER_AMPM__" },
	}
...
local function render_obsidian_datetime(format)
	...
	return rendered:gsub("__RET2HELL_OBSIDIAN_LOWER_AMPM__", function()
		return os.date("%p"):lower()
	end)
end

function M.patch_template_substitutions()
	if vim.g.ret2hell_obsidian_template_patch_applied then
		return
	end
	...
	templates.substitute_template_variables = function(text, client, note)
		text = text:gsub("{{date:([^}]+)}}", function(fmt)
			return render_obsidian_datetime(fmt)
		end)
	...
	vim.g.ret2hell_obsidian_template_patch_applied = true
end
...
function M.workspaces()
	...
	local workspaces = {
		{
			name = "obsidian",
			path = env_or("OBSIDIAN_VAULT", "~/Documents/obsidian"),
		},
	}
...
function M.opts()
	...
	return {
		...
		note_frontmatter_func = function(note)
			if note.title and not note.title:match("{{") then
				local exists = false
